...
df_raw_train = pd.read_csv("HW1_dataset//train-100-10.csv")

#import csv file for test-100-10
df_raw_test = pd.read_csv("HW1_dataset//test-100-10.csv")
...
df_x_train = df_raw_train.drop(["y", "Unnamed: 11", "Unnamed: 12"], axis=1)
...
# mse_graph.to_csv("mse_test_100_10.csv", index = False)
...
plt.plot(min_x, min_y, marker="o", color='red', label=f"min ({min_x}, {round(min_y,3)})")
plt.legend()
plt.xlabel("Lambda")
plt.ylabel("MSE")
plt.title("MSE graph for train-100-10 and test-100-10")
